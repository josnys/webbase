...
function CreateRole() {
     ...
     function handleSubmit(e) {
          e.preventDefault();
          setSending(true);
          Inertia.post(route('user.post.store', data.user.id), values).then(() => {
               setSending(false);
          });
     }
               ...
               <ProfileCard>
                    <div className="md:col-span-1">
                         <div className="px-4 sm:px-0">
                              <h3 className="text-md font-medium text-gray-900">Assign Store to User :</h3>
                              <h3 className="text-lg font-medium text-gray-700 mt-3">{data.user.name}</h3>
                              <p className="mt-1 text-sm text-gray-600">
                                   Assigning stores to user will provide them with ability to manage different aspect of the specific store
                              </p>
                         </div>
                    </div>
                    <DataCard>
                                   ...
                                   <div className="grid grid-cols-3">
                                        ...
                                        <div className="sm:col-span-4">
                                             ...
                                             <ul className="overflow-auto h-64 list-none pt-2 mt-2">
                                                  {data.stores.map(({id, name, type, isCheck}) => {
                                                       return <li className={`p-2 mt-2 rounded hover:bg-gray-200 hover:shadow-md ${(isCheck?'bg-blue-200':'')}`} key={id}>
                                                            ...
                                                            <p className="text-sm text-gray-500 pl-6">{type}</p>
                                                       </li>
                                                  })}
                                             </ul>
                                        </div>
